...
private:
  ...
  unsupportedConstructName(Token tok) -> std::string_view {
  ...
  [[nodiscard]] auto
  looksLikePropertyAssertion(size_t offset = 0) const -> bool {
  ...
  [[nodiscard]] auto
  structuralStateCompatible(const ParserState &lhs,
                            const ParserState &rhs) const -> bool {
...
[[nodiscard]] auto
TreeUnwrapper::unwrapWithDiagnostics() const -> UnwrapResult {
